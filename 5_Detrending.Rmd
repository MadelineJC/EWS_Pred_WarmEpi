...
width <- 3 
smoothed.const.df <- matrix(nrow = length(all.days), ncol = ncol(const.df)) 
smoothed.warm.df <- matrix(nrow = length(all.days), ncol = ncol(warm.df))
for (j in 1:ncol(const.df)){
  ...
  smoothed.const.df[ , j] <- smoothed.c
  smoothed.warm.df[ , j] <- smoothed.w
}
...
for (i in 1:1000){
  ...
  lines(1:120, smoothed.const.df[ , i], col = "cornflowerblue", lty = 2)
}
plot(0, 0, type = "n", xlim = c(1, 120), ylim = c(0, 120),
     main = "Rising Temperature", xlab = "Day", ylab = "Infecteds")
for (i in 1:1000){
  lines(1:120, warm.df[ , i], col = "grey")
  lines(1:120, smoothed.warm.df[ , i], col = "red3", lty = 2)
}
