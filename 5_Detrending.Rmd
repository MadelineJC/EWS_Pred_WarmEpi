...
widths <- seq(1, 8, 1); df <- matrix(nrow = length(all.days), ncol = length(widths))
for (j in widths){
  temp <- c(); smoothed <- c()
  for (i in 1:length(all.days)){
...
plot(sub8$day, sub8$infecteds, type = "l",
     main = "Manually Smoothed (Everyday), Varying Bandwidths", xlab = "Day", ylab = "Infecteds",
     col = "grey", lwd = 2)
for (i in 1:length(widths)){
  lines(all.days, df[ , i], col = cols[i], lwd = 1)
}
...
plot(sub8$day, sub8$infecteds, type = "l",
     main = "Manually Smoothed (Every 3 Days), Varying Bandwidths", xlab = "Day", ylab = "Infecteds",
     col = "grey")
for (i in 1:length(widths)){
  lines(sub8$day, df[ , i], col = cols[i], lty = 2)
}
```

Looking at the smoothed time series above, it looks like we should go with a **bandwidth of 2 or 3**.

# Smoothing Simulated Data
...
const.list <- list(constant1, constant2, constant3, constant4,
...
warm.list <- list(warming1, warming2, warming3, warming4,
                  warming5, warming6, warming7, warming8)
...
width <- 3; const.df <- matrix(nrow = length(all.days), ncol = 100) # Let's just do 100 pop'ns for now
constant1.mod <- constant1[ , 2:ncol(constant1)]
for (j in 1:100){
  temp <- c(); smoothed <- c()
  for (i in 1:length(all.days)){
    temp <- exp(-(day.nums[ , i]^2) / (2*(width^2)))
    temp <- temp/sum(temp)
    smoothed[i] <- sum(constant1.mod[ , j]*temp)
  }
  const.df[ , j] <- smoothed
}

plot(0, 0, type = "n", xlim = c(1, 120), ylim = c(0, 200),
     main = "", xlab = "", ylab = "")
for (i in 1:100){
  lines(1:120, constant1.mod[ , i], col = "grey")
  lines(1:120, const.df[ , i], col = "cornflowerblue", lty = 2)
}

width <- 3; warm.df <- matrix(nrow = length(all.days), ncol = 100) # Let's just do 100 pop'ns for now
warming1.mod <- warming1[ , 2:ncol(warming1)]
for (j in 1:100){
  temp <- c(); smoothed <- c()
  for (i in 1:length(all.days)){
    temp <- exp(-(day.nums[ , i]^2) / (2*(width^2)))
    temp <- temp/sum(temp)
    smoothed[i] <- sum(warming1.mod[ , j]*temp)
  }
  warm.df[ , j] <- smoothed
}
...
plot(0, 0, type = "n", xlim = c(1, 120), ylim = c(0, 200),
     main = "", xlab = "", ylab = "")
for (i in 1:100){
  lines(1:120, warming1.mod[ , i], col = "grey")
  lines(1:120, warm.df[ , i], col = "red3", lty = 2)
}
```

# Smoothing Empirical Data
...
all.days <- seq(1, 120, 1); exp.days <- seq(3, 120, 3); missing.days <- setdiff(all.days, exp.days)
missing.days <- data.frame(cbind(missing.days, c(rep(0, length(missing.days))))); colnames(missing.days) <- c("day", "infecteds")
sub1.mod <- rbind(sub1, missing.days)
sub1.mod <- sub1.mod[order(sub1.mod$day), ]
sub2.mod <- rbind(sub2, missing.days)
sub2.mod <- sub2.mod[order(sub2.mod$day), ]
sub3.mod <- rbind(sub3, missing.days)
sub3.mod <- sub3.mod[order(sub3.mod$day), ]
sub4.mod <- rbind(sub4, missing.days)
sub4.mod <- sub4.mod[order(sub4.mod$day), ]
sub5.mod <- rbind(sub5, missing.days)
sub5.mod <- sub5.mod[order(sub5.mod$day), ]
sub6.mod <- rbind(sub6, missing.days)
sub6.mod <- sub6.mod[order(sub6.mod$day), ]
sub7.mod <- rbind(sub7, missing.days)
sub7.mod <- sub7.mod[order(sub7.mod$day), ]
sub8.mod <- rbind(sub8, missing.days)
sub8.mod <- sub8.mod[order(sub8.mod$day), ]
subpops.list <- list(sub1.mod, sub2.mod, sub3.mod, sub4.mod,
                     sub5.mod, sub6.mod, sub7.mod, sub8.mod)
# Setting up the df for the top half of the expression
day.nums <- seq(0, length(all.days) - 1, 1)
for (i in 2:(length(all.days) - 1)){
  temp <- c(seq(-i + 1, -1, 1), 0, seq(1, length(all.days) - i, 1))
...
day.nums <- cbind(day.nums, rev(seq(0, length(all.days) - 1, 1))*(-1))
# Calculating the Gaussian kernel
width <- 1; subpops <- seq(1, 8, 1); df <- matrix(nrow = length(all.days), ncol = length(subpops))
for (j in subpops){
  temp <- c(); smoothed <- c()
  for (i in 1:length(all.days)){
    temp <- exp(-(day.nums[ , i]^2) / (2*(width^2)))
    temp <- temp/sum(temp)
    smoothed[i] <- sum(subpops.list[[j]]$infecteds*temp)
  }
  df[ , j] <- smoothed
}

cols <- c("navyblue", "steelblue3", "cornflowerblue", "lightblue", "darkred", "firebrick", "red3", "red2")
plot(0, 0, type = "n", xlim = c(1, 120), ylim = c(0, 5),
     main = "Constant Temperature", xlab = "Day", ylab = "Infecteds")
for (i in 1:4){
  lines(all.days, subpops.list[[i]]$infecteds, col = "grey", lwd = 2)
  lines(all.days, df[ , i], col = cols[i], lwd = 2)
}
plot(0, 0, type = "n", xlim = c(1, 120), ylim = c(0, 5),
...
for (i in 1:4){
  ...
  lines(sub1[ , 1], df[ , i], col = cols[i], lwd = 2)
}
plot(0, 0, type = "n", xlim = c(1, 120), ylim = c(0, 5),
     main = "Rising Temperature", xlab = "Day", ylab = "Infecteds")
for (i in 5:8){
...
# Plotting
```{r}
png("Plots/EmpDat_Detrend.png", width = 2000, height = 800)
par(mfrow = c(2, 4))
plot(sub1[ , 1], subpops.list[[1]][ , 2], type = "l", ylim = c(0, 5), col = "grey", lwd = 2,
     main = "Subpopulation 1 (Constant) | Every 3 Days; Bandwidth = 3", xlab = "Day", ylab = "Infecteds")
lines(sub1[ , 1], df[ , 1], col = "cornflowerblue", lwd = 2)
plot(sub2[ , 1], subpops.list[[2]][ , 2], type = "l", ylim = c(0, 5), col = "grey", lwd = 2,
     main = "Subpopulation 2 (Constant) | Every 3 Days; Bandwidth = 3", xlab = "Day", ylab = "Infecteds")
lines(sub2[ , 1], df[ , 2], col = "cornflowerblue", lwd = 2)
plot(sub3[ , 1], subpops.list[[3]][ , 2], type = "l", ylim = c(0, 5), col = "grey", lwd = 2,
     main = "Subpopulation 3 (Constant) | Every 3 Days; Bandwidth = 3", xlab = "Day", ylab = "Infecteds")
lines(sub3[ , 1], df[ , 3], col = "cornflowerblue", lwd = 2)
plot(sub4[ , 1], subpops.list[[4]][ , 2], type = "l", ylim = c(0, 5), col = "grey", lwd = 2,
     main = "Subpopulation 4 (Constant) | Every 3 Days; Bandwidth = 3", xlab = "Day", ylab = "Infecteds")
lines(sub4[ , 1], df[ , 4], col = "cornflowerblue", lwd = 2)
plot(sub5[ , 1], subpops.list[[5]][ , 2], type = "l", ylim = c(0, 5), col = "grey", lwd = 2,
     main = "Subpopulation 5 (Warming) | Every 3 Days; Bandwidth = 3", xlab = "Day", ylab = "Infecteds")
lines(sub5[ , 1], df[ , 5], col = "red3", lwd = 2)
plot(sub6[ , 1], subpops.list[[6]][ , 2], type = "l", ylim = c(0, 5), col = "grey", lwd = 2,
     main = "Subpopulation 6 (Warming) | Every 3 Days; Bandwidth = 3", xlab = "Day", ylab = "Infecteds")
lines(sub6[ , 1], df[ , 6], col = "red3", lwd = 2)
plot(sub7[ , 1], subpops.list[[7]][ , 2], type = "l", ylim = c(0, 5), col = "grey", lwd = 2,
     main = "Subpopulation 7 (Warming) | Every 3 Days; Bandwidth = 3", xlab = "Day", ylab = "Infecteds")
lines(sub7[ , 1], df[ , 7], col = "red3", lwd = 2)
plot(sub8[ , 1], subpops.list[[8]][ , 2], type = "l", ylim = c(0, 5), col = "grey", lwd = 2,
     main = "Subpopulation 8 (Warming) | Every 3 Days; Bandwidth = 3", xlab = "Day", ylab = "Infecteds")
lines(sub8[ , 1], df[ , 8], col = "red3", lwd = 2)
dev.off()
```